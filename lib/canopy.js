const individual = require('individual')
const levels = ['debug', 'info', 'warn', 'error']

const canopyConfig = individual('__CANOPY_CONFIG', {
  outputStream: process.stdout,
  dateProvider: () => new Date(),
  errorHandlers: []
})

class CanopyUsageError extends Error {
  constructor(message) {
    super(message)
    this.name = 'CanopyUsageError'
    this.message = message
  }
}

const _parseArgs = (a1, a2) => {
  let logName
  let obj = {}

  if (a2 !== undefined) {
    if (typeof (a1) !== 'string' && a1 !== undefined) {
      throw new TypeError('When providing two arguments, first argument must be a string or undefined')
    }
    if (typeof (a2) !== 'object') {
      throw new TypeError('When providing two arguments, second argument must be an object')
    }
    logName = a1
    obj = a2
  } else if (a1 !== undefined) {
    if (typeof (a1) === 'string') {
      logName = a1
    } else if (typeof (a1) === 'object') {
      obj = a1
    } else {
      throw new TypeError('Argument must be a name or object', a1)
    }
  } else {
    logName = undefined
    obj = {}
  }
  return [logName, obj]
}

function canopy (a1, a2) {
  const [logName, obj] = _parseArgs(a1, a2)

  function canopyLogger (a1, a2) {
    const [newNamePart, newObjParts] = _parseArgs(a1, a2)
    const parts = [ logName, newNamePart ].filter(Boolean)
    const newName = parts.join(':') || undefined

    const newObj = {}
    for (const k in (obj || {})) {
      newObj[k] = obj[k]
    }
    for (const k in (newObjParts || {})) {
      newObj[k] = newObjParts[k]
    }

    return canopy(newName, newObj)
  }

  canopyLogger.logName = logName
  canopyLogger.obj = obj

  for (const level of levels) {
    /* Note: The order that the loggable object is constructed is tweaked so
     * that the most interesting properties appear first when executing under
     * Node.
     */
    canopyLogger[level] = function (a1, a2) {
      const loggable = { timestamp: canopyConfig.dateProvider().toISOString() }
      if (this.logName !== undefined) {
        loggable.name = this.logName
      }
      loggable.severity = level

      try {
        if (typeof (a1) === 'string') {
          loggable.message = a1

          if (typeof (a2) === 'object') {
            for (const k in a2) {
              loggable[k] = a2[k]
            }
          } else if (a2 !== undefined) {
            throw new CanopyUsageError('When logging a string message, the second argument must be an obejct if provided')
          }
        } else if (a1 instanceof Error) {
          // Messages and objects are logegd before the error for readability.
          if (typeof (a2) === 'string') {
            loggable.message = a2
          } else if (typeof (a2) === 'object') {
            for (const k in a2) {
              loggable[k] = a2[k]
            }
          } else if (a2 !== undefined) {
            throw new CanopyUsageError('When logging an error, the second argument must be a string or object if provided')
          }

          loggable.err = {
            name: a1.name,
            message: a1.message
          }
          // Log the code if we have one.
          if (a1.code) {
            loggable.err.code = a1.code
          }
          loggable.err.stack = a1.stack

          for (const handler of canopyConfig.errorHandlers) {
            handler(a1)
          }
        } else if (typeof (a1) === 'object') {
          if (a2 !== undefined) {
            throw new CanopyUsageError('When logging an object, there can be no other arguments')
          }
          for (const k in a1) {
            loggable[k] = a1[k]
          }
        } else if (a1 === undefined) {
          throw new CanopyUsageError('Log message, object or error not provided')
        } else {
          throw new CanopyUsageError('Unsupported argument')
        }
      } catch (e) {
        return canopyLogger[level](e)
      }

      canopyConfig.outputStream.write(JSON.stringify(loggable) + '\n')
    }
  }

  return canopyLogger
}

module.exports = canopy
module.exports.setOutputStream = (stream) => {
  canopyConfig.outputStream = stream
}
module.exports.setDateProvider = (dateProvider) => {
  canopyConfig.dateProvider = dateProvider
}
module.exports.addErrorHandler = (handler) => {
  canopyConfig.errorHandlers.push(handler)
}
module.exports.quiet = () => {
  canopyConfig.outputStream = { write: () => {} }
  canopyConfig.errorHandlers = []
}
